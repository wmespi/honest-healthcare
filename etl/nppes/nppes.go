...
func Run(nppesURL, nppesFile string, limit int) {
	...
	if strings.HasSuffix(strings.ToLower(nppesFile), ".csv") {
		...
		defer f.Close()
		kept, err := extractNPPESGeorgia(f, core.NPPESOutputPath, limit)
		if err != nil {
			log.Fatalf("❌ %v", err)
		}
		log.Printf("✅ Wrote %s — %d Georgia providers", core.NPPESOutputPath, kept)
		return
	}
	...
	if zipPath == "" {
		...
		if err := core.DownloadToFile(nppesURL, zipPath); err != nil {
			cleanup()
	...
	kept, err := extractNPPESGeorgia(rc, core.NPPESOutputPath, limit)
	if err != nil {
		log.Fatalf("❌ %v", err)
	}
	log.Printf("✅ Wrote %s — %d Georgia providers", core.NPPESOutputPath, kept)
...
func extractNPPESGeorgia(r io.Reader, outPath string, limit int) (int, error) {
	...
	defer f.Close()
	pw := parquet.NewGenericWriter[core.NPPESRow](f, parquet.Compression(&parquet.Zstd))
	defer pw.Close()

	buf := make([]core.NPPESRow, 0, 4096)
	kept, scanned := 0, 0
	for {
		rec, err := cr.Read()
		if err == io.EOF {
