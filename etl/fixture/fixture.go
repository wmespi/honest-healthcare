...
// FixtureDir is where committed fixtures live — read by the extraction package's
// regression tests. Relative to the etl/ working dir.
const FixtureDir = "extraction/testdata/fixtures"
...
func Make(ctx context.Context, conn *pgx.Conn, fileID int, fixtureURL, name string) {
	url := fixtureURL
	if url == "" {
	...
	gz, err := gzip.NewReader(core.NewProgressReader(resp.Body, resp.ContentLength))
	if err != nil {
